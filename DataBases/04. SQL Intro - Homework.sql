...
SQL consists of:
 ...
 --- Data Definition Language (DDL)
	CREATE, DROP, ALTER
	GRANT, REVOKE
2. What is Transact-SQL (T-SQL)? - T-SQL (Transact SQL) is an extension to the standard SQL language. T-SQL is the standard language used in MS SQL Server. 
...
SELECT (FirstName + '.' + LastName + '@telerik.com') [Full Email Addresses] FROM Employees
...
SELECT e.FirstName + ' ' + e.LastName AS Name, a.AddressText FROM Employees e
JOIN Addresses a ON e.AddressID = a.AddressID

--19.Write a SQL query to find all employees and their address. Use equijoins (conditions in the WHERE clause).
SELECT e.FirstName + ' ' + e.LastName AS Name, a.AddressText FROM Employees e, Addresses a 
...
SELECT e.FirstName + ' ' + e.LastName AS EmployeeName, m.FirstName + ' ' + m.LastName AS ManagerName, a.AddressText FROM Employees e
JOIN Employees m ON e.ManagerID = m.EmployeeID
JOIN Addresses a ON e.AddressID = a.AddressID
...
SELECT e.FirstName + ' ' + e.LastName AS EmployeeName, m.FirstName + ' ' + m.LastName AS Manager FROM Employees e
LEFT JOIN Employees m ON e.ManagerID = m.EmployeeID

--24.Write a SQL query to find the names of all employees from the departments "Sales" and "Finance" whose hire year is between 1995 and 2005.
SELECT e.FirstName + ' ' + e.LastName AS EmployeeName, d.Name, e.HireDate FROM Employees e
